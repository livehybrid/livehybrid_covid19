...
SCHEME = """<scheme>
    <title>Covid-19 CDC Stats - US Regions</title>
    <description>Poll for Covid-19 US Regional case stats</description>
...
def do_run():
    ...
    try:
        ...
        while True:
            ...
            resp = requests.get(url="https://www.cdc.gov/coronavirus/2019-ncov/map-data-cases.csv", params=req_args)
            reader = csv.DictReader(resp.iter_lines())
            for record in reader:
                empty_keys = [k for k,v in record.iteritems() if not v]
                #Tidy some fields
                record["Community_Transmission"] = record["Community Transmission\x86"]
                del record["Community Transmission\x86"]

                

                for k in empty_keys:
                    del record[k]
                print_xml_stream(json.dumps(record,ensure_ascii=False),"map-data-cases.csv")
